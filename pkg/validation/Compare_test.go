		...
		g.Describe(`array`, func() {
			g.Describe(`array:numeric`, func() {
				g.It("success when the element value equals the expected number", func() {
					...
					result := compare("eachEq", proto, value)
					g.Assert(result).Equal("")
				})
		...
		g.Describe(`array`, func() {
			g.Describe(`array:numeric`, func() {
				g.It("success when the element value exceeds the min threshold", func() {
					...
					result := compare("eachMin", proto, value)
					g.Assert(result).Equal("")
				})
		...
		g.Describe(`array`, func() {
			g.Describe(`array:numeric`, func() {
				g.It("success when the element value is less than the max threshold", func() {
					...
					result := compare("eachMax", proto, value)
					g.Assert(result).Equal("")
				})
...
func TestCompareMinFields(t *testing.T) {
	...
	g.Describe(`Rule "minFields"`, func() {
		g.It("success when the number of fields exceeds the minimum threshold", func() {
			proto := reflect.ValueOf(4)
			value := reflect.ValueOf(8)

			result := compare("minFields", proto, value)
			g.Assert(result).Equal("")
		})

		g.It("success when the number of fields reaches the minimum threshold", func() {
			proto := reflect.ValueOf(4)
			value := reflect.ValueOf(4)
		...
		g.It("failure when the number of fields is less than the minimum threshold", func() {
			proto := reflect.ValueOf(10)
			value := reflect.ValueOf(4)

			result := compare("minFields", proto, value)
			g.Assert(result).Equal("must contain at least 10 valid fields")
		})

		g.It("failure when given invalid threshold", func() {
			proto := reflect.ValueOf(nil)
